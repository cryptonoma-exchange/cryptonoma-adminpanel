				...
				<div class="row">
					<div class="col-md-6 tg-select-left">
						<select class="form-control custom-s-left" onchange="pageredirect(this)">
							<option value="{{ url('admin/buy_tradehistory/' . $tradepair->coinone . '_' . $tradepair->cointwo . '/limit') }}"
								@if ($order_type == 1) selected @endif>Limit</option>
							<option value="{{ url('admin/buy_tradehistory/' . $tradepair->coinone . '_' . $tradepair->cointwo . '/market') }}"
					...
					<div class="col-md-6 tg-select">
						...
						<select onchange="location = this.value;" class="form-control custom-s">
							@if (isset($pairs))
								<option value="{{ url('admin/buy_tradehistory/' . $tradepair->coinone . '_' . $tradepair->cointwo . '/' . $type) }}">
								...
								@foreach ($pairs as $coinones)
									@if ($coinones->coinone . '_' . $coinones->cointwo != $tradepair->coinone . '_' . $tradepair->cointwo)
										<option value="{{ url('admin/buy_tradehistory/' . $coinones->coinone . '_' . $coinones->cointwo . '/' . $type) }}">
